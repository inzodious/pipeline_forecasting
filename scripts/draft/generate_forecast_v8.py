...
def build_stage_probabilities(df):
    """Calculates win rates for deals that reached each stage."""
    # Only look at deals that have reached a final outcome
    exits = df[df['is_closed']].copy()
    
    # Apply trailing 12 month weighting for relevance
    cutoff_date = exits['date_snapshot'].max()
    t12_start = cutoff_date - pd.DateOffset(months=12)
    exits['weight'] = np.where(exits['date_snapshot'] >= t12_start, 3.0, 1.0)
    ...
    if not probs.empty:
        ...
        probs['prob'] = np.maximum(probs['prob'], probs['global_prob'] * 0.95) # Anchor
...
def forecast_future_pipeline(deals, start_date, end_date, levers=None):
    ...
    # 1. Volume: T12M Monthly Average with Skipper Weighting
    ...
    vol_base = vol_agg.groupby('market_segment')['volume_weight'].mean().reset_index(name='avg_monthly_vol')
    
    # 2. Win Rate: Won Revenue / Total Created Revenue (Cohort Analysis)
    # Using T12M deals to ensure we capture recent business performance
    def get_metrics(df_seg):
        won = df_seg[df_seg['won']]
        # Win Rate as % of Won Rev / Total Pipeline Rev
        # CRITICAL: We weight by volume_weight to align with volume creation
        wr = (won['net_revenue'] * won['volume_weight']).sum() / (df_seg['net_revenue'] * df_seg['volume_weight']).sum() if (df_seg['net_revenue'] * df_seg['volume_weight']).sum() > 0 else 0
        avg_size = won['net_revenue'].mean() if len(won) > 0 else 0
        return pd.Series({'win_rate': wr, 'avg_size': avg_size})
    ...
    metrics = vol_base.merge(wr_t12, on='market_segment', how='left', suffixes=('_t12', '_all'))
    metrics = metrics.merge(wr_all, on='market_segment', how='left', suffixes=('', '_all_temp'))
    # Rename for clarity
    metrics = metrics.rename(columns={'win_rate': 'win_rate_t12', 'avg_size': 'avg_size_t12', 
                                      'win_rate_all_temp': 'win_rate_all', 'avg_size_all_temp': 'avg_size_all'})
    
    # Credibility blending for win rate and size
    metrics['win_rate'] = (metrics['win_rate_t12'] * 0.55) + (metrics['win_rate_all'] * 0.45)
    metrics['avg_size'] = (metrics['avg_size_t12'] * 0.55) + (metrics['avg_size_all'] * 0.45)
...
def run_backtest(snapshots):
    ...
    summary = forecast_combined.groupby('market_segment')['expected_revenue'].sum().reset_index(name='forecast')
    ...
    if not act_prewon.empty:
        ap_sum = act_prewon.groupby('market_segment')['net_revenue'].sum().reset_index(name='prewon')
        summary = summary.merge(ap_sum, on='market_segment', how='left').fillna(0)
        summary['forecast'] += summary['prewon']
    
    print(f"Total Won Deals in Actuals: {len(all_act_won)}")
    ...
    res['var_pct'] = (res['forecast'] / res['actual'] - 1) * 100
    
    print(res)
    print(f"\nTotal Forecast (incl. pre-won reconciliation): ${res['forecast'].sum():,.2f}")
    print(f"Total Actual: ${res['actual'].sum():,.2f}")
    return res
